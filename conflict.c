...
void
conflict (int shipno, int planetno)
{
...
/* This assumes that if there are ships on an unclaimed and unowned planet,
   they must be scouts.  Seems like a good assumpton for the time being... */
					...
					for (j=0; j < num_def_ships; j++) {

						register struct ship *s2;
						
						if (def_ships[j] == -1) continue;

						s2 = &game.ships[def_ships[j]];
		...
		case 'b' : {
			int def, j;
			float mult_ship = (1 + ((float)(rnd(20) - 10) / 100));
			float mult_plan = (1 + ((float)(rnd(20) - 5) / 100));
			...
			for (j=0; j < num_def_ships; j++) {
				...
				def = s->s_points;
				s->s_points -= (int) (s2->s_points * mult_ship);
				s2->s_points -= (int) (def * mult_plan);
				if (s2->s_points <= 0) {
					s2->s_points = 0;
			...
			def = s->s_points;
			s->s_points -= (int) (p->p_def * mult_ship);
			p->p_def -= (int) (def * mult_plan);
			...
			if (p->p_def > 0) {
				...
				if (p->p_emp != -1)
					logmsg (p->p_emp, "Planet #%02d DESTROYED!\n", planetno);
				p->p_emp = -1;
				p->p_tech -= (int) (def * mult_plan / 10);
				p->p_util -= (int) (def * mult_plan / 3);
				p->p_prod -= (int) (def * mult_plan / 4);
				p->p_res  -= (int) (def * mult_plan * 7);
